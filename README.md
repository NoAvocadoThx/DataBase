...
| AI 助手 | `/chat` 对话式问库：模型通过工具调用真去查专家 / 会议 / 统计，流式回复，引用的专家带 `/expert/{id}` 链接；多轮上下文只存在浏览器里，刷新即清空 | 所有人 |
...
AI 助手的工具结果走字段白名单（手机 / 邮箱 / 微信 / 内部备注 / 关注说明 / 录入原文一律不出境），出境前再整体打一次码；每次提问写访问日志；单人每分钟最多 10 次提问。未配置 `LLM_API_KEY` 时对话框给出提示并引导去「找专家」，不报错。
...
- `tests/test_chat.py` AI 对话（未配置降级、外发脱敏、工具调用查库、实习生权限、频率限制、访问留痕、上下文截断、网络异常兜底；大模型调用全部 mock，不产生费用）
